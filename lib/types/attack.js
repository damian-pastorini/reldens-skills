...
class Attack extends Skill
{
    ...
    constructor(props)
    {
        ...
        let affectedProperty = sc.get(props, 'affectedProperty', false);
        if(!affectedProperty){
    ...
    calculateCriticalDamage(damage, targetDodge, ownerAim, dodgeAimDiff)
    {
        ...
        // if criticalAffected is false OR dodge is NOT greater than aim, return full critical value
        // if criticalAffected is true AND dodge > aim, apply dodge reduction to critical
        if(!this.criticalAffected || targetDodge <= ownerAim){
            return criticalValue;
        }
        this.lastState = SkillsConst.SKILL_STATES.APPLIED_CRITICAL_DAMAGE;
        // calculate the dodge proportion over critical damage and remove it from the critical total:
    ...
    calculateProportionDamage(target, damage, targetDodge, ownerAim, dodgeAimDiff)
    {
        ...
        if(this.damageAffected && targetDodge > ownerAim){
            // calculate a dodge proportion over damage and remove it from the total:
    ...
    getPropertiesTotal(objectInstance, propertiesArray)
    {
        if(!objectInstance){
        ...
        try {
            ...
            for(let prop of propertiesArray){
                let propValue = this.propertyManager.getPropertyValue(objectInstance, prop);
    ...
    {
        // prevent division by zero when totalValue (for example, "aim") is 0:
        if(0 === totalValue){
            // if both "aim" and "dodge" are 0, no advantage either way so we return 0% reduction:
            if(0 === upProportionValue){
                return 0;
            }
            // if aim is 0 but dodge > 0, target has maximum advantage so we return 100% reduction:
            return 100;
        }
    ...
    setAffectedPropertyValue(target, value)
    {
        return this.propertyManager.setOwnerProperty(target, this.affectedProperty, value);
    }
