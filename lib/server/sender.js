...
class Sender
{
    ...
    registerListeners()
    {
        ...
        this.classPath.listenEvent(SkillsEvents.SKILL_BEFORE_CAST, async (skill, target) => {
            await this.onSkillBeforeCast(skill, target);
        }, 'skillBeforeCastSender', this.getEventMasterKey());
    }

    async onInitClassPathEnd(classPath)
    {
        let messageData = {
    ...
    async onLevelExperienceAdded(classPath)
    {
        let messageData = {
            exp: classPath.currentExp
        };
    ...
    async onSkillBeforeCast(skill, target)
    {
