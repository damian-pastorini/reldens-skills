...
class Sender
{
    ...
    registerListeners()
    {
        ...
        this.classPath.listenEvent(SkillsEvents.SKILL_ATTACK_APPLY_DAMAGE, async (skill, target, newValue) => {
            await this.onSkillAttachApplyDamage(skill, target, newValue);
        }, 'skillAttackApplyDamageSender', this.getEventMasterKey());
        // @TODO - BETA.17 - CHECK: skill cast, damage were missing, do we need to include other events?
    }
    ...
    async onSkillAttachApplyDamage(skill, target, newValue)
    {
        let damage = skill.propertyManager.getPropertyValue(target, skill.affectedProperty) - newValue;
        let messageData = {
            d: damage
        };
        if(typeof skill.owner.getSkillExtraData === 'function'){
