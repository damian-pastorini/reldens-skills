...
const { rawRegisteredEntities } = require('./models/registered-entities');
...
class ModelsManager
{
    ...
    constructor(props)
    {
        let dataServerConfig = sc.getDef(props, 'dataServerConfig', {});
        dataServerConfig.rawEntities = rawRegisteredEntities;
        this.dataServer = sc.hasOwn(props, 'dataServer') ? props.dataServer
            : new ObjectionJsDataServer(dataServerConfig);
        if(!this.dataServer.initialized){
            this.dataServer.connect();
        }
    ...
    async loadOwnerClassPath(ownerId)
    {
        ...
        return this.dataServer.entityManager.get('ownersClassPath')
            .loadByWithRelations('owner_id', ownerId, '[owner_full_class_path]');
    }
