...
class Skill
{
    ...
    {
        this.fireEvent(SkillsEvents.SKILL_BEFORE_EXECUTE, this, target);
        if(target){
            this.target = target;
        }
        if(!this.target){
        ...
        if(this.castTime > 0){
            this.fireEvent(SkillsEvents.SKILL_BEFORE_CAST, this, target);
        ...
        this.fireEvent(SkillsEvents.SKILL_AFTER_EXECUTE, this, target);
        return skillLogicResult;
    }

    finishExecution(target)
    {
        this.fireEvent(SkillsEvents.SKILL_BEFORE_RUN_LOGIC, this, target);
        if(this.ownerEffects){
            this.applyModifiers(this.ownerEffects, this.owner);
        }
        let skillLogicResult = this.runSkillLogic();
        this.fireEvent(SkillsEvents.SKILL_AFTER_RUN_LOGIC, this, target);
        return skillLogicResult;
    }
    ...
    {
        return 'p'+this.getOwnerId()+'.'+eventName;
    }
