...
class ClassPath extends LevelsSet
{
    ...
    init(props)
    {
        ...
        this.fireEvent(SkillsEvents.INIT_CLASS_PATH_END, this);
    }
    ...
    levelUp()
    {
        ...
        if(sc.hasOwn(this.skillsByLevel, this.currentLevel)){
            this.addSkills(this.skillsByLevel[this.currentLevel]);
        }
    }
    ...
    levelDown()
    {
        super.levelDown();
        if(sc.hasOwn(this.skillsByLevel, this.currentLevel)){
            this.removeSkills(this.skillsByLevel[this.currentLevel]);
        }
    }

    addSkills(skills)
    {
        this.fireEvent(SkillsEvents.ADD_SKILLS_BEFORE, this, skills);
        for(let i of Object.keys(skills)){
            let skill = skills[i];
            this.currentSkills[skill.key] = skill;
        }
        this.fireEvent(SkillsEvents.ADD_SKILLS_AFTER, this, skills);
    }

    removeSkills(skills)
    {
        this.fireEvent(SkillsEvents.REMOVE_SKILLS_BEFORE, this, skills);
        for(let skillKey of skills){
            delete this.currentSkills[skillKey];
        }
        this.fireEvent(SkillsEvents.REMOVE_SKILLS_AFTER, this, skills);
    }
    ...
    setOwnerSkills()
    {
        for(let i of Object.keys(this.levels)){
            ...
            if(this.skillsByLevel && sc.hasOwn(this.skillsByLevel, level.key) && level.key <= this.currentLevel){
                this.addSkills(this.skillsByLevel[level.key]);
            }
        }
        this.fireEvent(SkillsEvents.SET_SKILLS, this);
    }
