...
class ClassPath extends LevelsSet
{
    ...
    addSkills(skills)
    {
        this.events.emit(SkillsEvents.ADD_SKILLS_BEFORE, this, skills);
        for(let i of Object.keys(skills)){
            let skill = skills[i];
            this.currentSkills[skill.key] = skill;
        }
        this.events.emit(SkillsEvents.ADD_SKILLS_AFTER, this, skills);
    }

    removeSkills(skills)
    {
        this.events.emit(SkillsEvents.REMOVE_SKILLS_BEFORE, this, skills);
        for(let skillKey of skills){
            delete this.currentSkills[skillKey];
        }
        this.events.emit(SkillsEvents.REMOVE_SKILLS_AFTER, this, skills);
    }
    ...
    setOwnerSkills()
    {
        ...
        this.events.emit(SkillsEvents.SET_SKILLS, this);
    }
