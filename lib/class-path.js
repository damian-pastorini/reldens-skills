...
class ClassPath extends LevelsSet
{
    ...
    async init(props)
    {
        ...
        let key = sc.get(props, 'key');
        if(!key){
            Logger.critical('Undefined key for class path.');
            return false;
        }
        ...
        this.label = sc.get(props, 'label', key);
    ...
    loopLevelsForLabel()
    {
        ...
        for(let i of Object.keys(this.labelsByLevel)){
            if(i <= this.currentLevel){
                label = this.labelsByLevel[i];
            }
        }
        return label;
    }
    ...
    async levelDown()
    {
        ...
        if(sc.hasOwn(this.skillsByLevel, this.currentLevel)){
    ...
    async removeSkills(skills)
    {
        ...
        for(let i of Object.keys(skills)){
            let skillOrKey = skills[i];
            let skillKey = 'string' === typeof skillOrKey ? skillOrKey : skillOrKey.key;
